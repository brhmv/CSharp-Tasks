...
class Debtor
{
    ...
    public override string ToString() => $"{FullName} {BirthDay.ToShortDateString()} {Phone} {Email} {Address} {Debt}";
}
...
class Program
{
    ...
    static void Main()
    {
        ...
        //result.ForEach(d => Console.WriteLine(d.FullName + d.Email));

        //2) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag
        var sortAge = debtors.Where(d => 26 < DateTime.Now.Year - d.BirthDay.DayOfYear && DateTime.Now.Year - d.BirthDay.Year < 36).ToList();
        //sortAge.ForEach(sa => Console.WriteLine(sa));

        //3) Borcu 5000 - den cox olmayan borclularic cixartmag
        var sortDebt5000 = debtors.Where(d => d.Debt < 5000).ToList();
        //sortDebt5000.ForEach(d => Console.WriteLine(d));

        //4) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq
        var sortName18 = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2).ToList();
        //sortName18.ForEach(d => Console.WriteLine(d));

        //5) Qishda anadan olan borclulari cixardmaq
        var sortMonthWinter = debtors.Where(d => d.BirthDay.Month == 1 || d.BirthDay.Month == 2 || d.BirthDay.Month == 12).ToList();
        //sortMonthWinter.ForEach(d => Console.WriteLine(d));
        ...
        var sortNumberWO8 = debtors.Where(d => d.Phone.Contains('8')).Select(d => d.FullName + "\n" + (DateTime.Now.Year - d.BirthDay.Day) + "\n" + d.Debt + "\n").ToList();
        //sortNumberWO8.ForEach(d => Console.WriteLine(d));
        ...
        // var sortFullName3 = debtors.Where(d => d.FullName.Count(c =>  c >= 3)).ToList();

        //9)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq
        var sortBigestDebt = debtors.Select(d => d.Debt).ToList();
        sortBigestDebt.Sort();
        var maxDebt = sortBigestDebt.Last();
        sortBigestDebt = debtors.Where(d => d.Debt == maxDebt).Select(d => d.BirthDay.DayOfYear).ToList();
        // Console.WriteLine("Year:" + sortBigestDebt);

        /****///10)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
        
        var sortBigestDebts5 = debtors.Select(d => d.Debt).ToList();
        
        sortBigestDebts5.Sort(); //debts list

       
        
        var maxDebt5 = sortBigestDebts5.Last(); //5

        var biggest5 = debtors.Where(d => d.Debt == maxDebt5).ToList();

        sortBigestDebts5.Remove(maxDebt5);
        /////////////////////////////////////////////

        var maxDebt4 = sortBigestDebts5.Last(); //4

        // var biggest4 = debtors.Where(d => d.Debt == maxDebt4).ToList();

        biggest5.Append(debtors.Where(d => d.Debt == maxDebt4));

        sortBigestDebts5.Remove(maxDebt4);
        
        //////////////////////////////////////////////
        var maxDebt3 = sortBigestDebts5.Last(); //3

        var biggest3 = debtors.Where(d => d.Debt == maxDebt3);

        sortBigestDebts5.Remove(maxDebt3);

        Console.WriteLine(biggest3);
        ...
        var totalDebt = debtors.Select(d => d.Debt).Sum();
        //Console.WriteLine(totalDebt);

        //12)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
        var sortAgeWar2 = debtors.Where(d => d.BirthDay.Year <= 1941).ToList();
        //sortAgeWar2.ForEach(d => Console.WriteLine(d));

        /****///13)Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
              //  var sortNumberNotRepated = debtors.Where(d => d.Phone.Distinct().Select(d => d.Debt).ToList();
        ...
        var sortPay500PerMonth = debtors.Where(d => 500 * (d.BirthDay.Month - DateTime.Now.Month) == d.Debt).ToList();
        //sortDebt5000.ForEach(d => Console.WriteLine(d));

        //15)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq
        var sortNameSmile = debtors.Where(d => d.FullName.Contains('s') && d.FullName.Contains('m') && d.FullName.Contains('i') && d.FullName.Contains('l') && d.FullName.Contains('e')).ToList();
        //sortNameSmile.ForEach(d => Console.WriteLine(d));
